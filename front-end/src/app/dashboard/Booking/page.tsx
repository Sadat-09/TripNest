...
const RoomSearch: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get<Booking[]>(
        `http://localhost:3000/user/search/${location}`
      );
  ...
  const handleBookhistory = async () => {
    try {
      const response = await axios.get<Booking[]>(
        `http://localhost:3000/user/bookingHistory`
      );
  ...
  const handleSubmitReview = async (
    ...
    try {
      await axios.post("http://localhost:3000/review/reviewrating", {
        hotelname: bookings.find((booking) => booking.id === reviewVisible)
          ?.Hotelname,
        location: bookings.find((booking) => booking.id === reviewVisible)
          ?.location,
        review,
        rating,
      });
    ...
    <>
      ...
      <div className="flex justify-center">
        <div className="w-full max-w-screen-md">
          <h1 className="text-3xl text-center text-blue-500 font-semibold mb-6">
            Book Room
          </h1>
          <div className="bg-white border border-blue-500 p-6 rounded-lg">
            ...
            {
              !showSearchResult && <h1>No Available Hotels</h1>
            }
